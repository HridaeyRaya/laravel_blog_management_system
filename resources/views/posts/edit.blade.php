...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <p class="text-gray-500 mt-1 truncate">{{ $post->title }}</p>
        </div>
                ...
                <form method="POST" action="{{ route('posts.update', $post->slug) }}">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="body" id="body" rows="12"
                                  class="w-full px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition resize-none @error('body') border-red-300 bg-red-50 @else border-gray-200 @enderror">{{ old('body', $post->body) }}</textarea>
                        @error('body')
                        <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
